...
// EncryptToString : value, key name
func EncryptToString(value string, key string) (string, error) {
	cipherKey := os.Getenv(key)

	if cipherKey == "" {
		return value, errors.New("Encrypt key empty")
	}
...
// DecryptToString : value, key name
func DecryptToString(value string, key string) (string, error) {
	cipherKey := os.Getenv(key)

	if cipherKey == "" {
		return value, errors.New("Decrypt key empty")
	}
	...
	isValid := []byte(returnMe)
	if !utf8.Valid(isValid) {
		panic("Decrypt to string invalid")
	}

	return returnMe, nil
}
func Decrypt2KeyToString(keyValue string, key string) (string, error) {
	cipherKey := os.Getenv(key)
	value := os.Getenv(keyValue)

	if cipherKey == "" || value == "" {
		return value, errors.New("Decrypt keys empty")
	}
